...
- `onsite-product-manager`: Internal alpha v0.0.1 for turning full customer-meeting evidence, operator focus, and customer context into an onsite product opportunity document with 3-10 materially different product propositions, validation experiments, and independent Build Cards before any prototype development.
- `onsite-prototype-development`: Internal alpha v0.0.3 for either framing one product direction from meeting evidence or executing one fixed upstream Build Card, then selecting a compatible registered Project Template, establishing a researched visual target when material, and building one clickable sales-demo loop while keeping mock and production boundaries explicit.
