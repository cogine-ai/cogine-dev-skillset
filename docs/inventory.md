...
This document records which local skills should be part of `cogine-dev-skillset`.
...
- `onsite-product-manager`: Internal alpha v0.0.2 for turning full customer-meeting evidence, operator focus, and customer context into a concise human-facing product-direction board, an internal context freeze, and 3-10 independent Build Cards before any prototype development.
- `onsite-prototype-development`: Internal alpha v0.0.4 for either framing one product direction from meeting evidence or executing one fixed upstream Build Card with its linked upstream context freeze, then selecting a compatible registered Project Template, establishing a researched visual target when material, and building one clickable sales-demo loop while keeping mock and production boundaries explicit.
